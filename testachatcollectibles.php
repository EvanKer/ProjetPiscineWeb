...
<body>
  ...
  <div class="container" text-align="center">
    ...
    <div class="col-lg-9">
      ...
      <div class="card mt-4">
        <p><?php echo '<img  src="'.$_SESSION['photo'].'" alt="PhotoItem">'?></p>
        <div class="card-body">
          ...
          <h4>Type de vente :
           ...
           $requete='SELECT * FROM `collectibles` WHERE nom= "'.$_SESSION['item'].'" ';
      ...
      <h4>DATE DE FIN  :
        ...
        $requete='SELECT `tempsRestant` FROM `collectibles` WHERE nom= "'.$_SESSION['item'].'" ';
        ...
        while ($ligne=$resultat->fetch_assoc()) {
         ...
         if ($dateActuel<=$ligne['tempsRestant'])
         {
          ...
          $sql ="UPDATE collectibles SET etat = '0' WHERE `collectibles`.`nom`= '".$_SESSION['item']."'";
          $result =mysqli_query($db_handle, $sql); 
...

        {
          ...
          $sql ="UPDATE collectibles SET etat = '1' WHERE `collectibles`.`nom` = '".$_SESSION['item']."'' ";
          $result =mysqli_query($db_handle, $sql);
        }
...
$db_handle = mysqli_connect('localhost', 'root', '');
$db_found = mysqli_select_db($db_handle, $database);
$requete= "SELECT prixActuel AND prix FROM collectibles ";
$resultat = mysqli_query($db_handle, $requete);
...
while ($ligne=$resultat2->fetch_assoc()) {
  ...
  if ($ligne['etat']=='0'){
    ...
    if($b1) { // faire une offre
      if ($db_found) {
        if ($prixActuel<$ligne['prixActuel'])
        {
          ...
          $sql = "SELECT prixActuel FROM collectibles ";
          $result = mysqli_query($db_handle, $sql);

          $sql ="UPDATE collectibles SET prixActuel = '$prixActuel' WHERE `collectibles`.`nom`= '".$_SESSION['item']."'";
          $result =mysqli_query($db_handle, $sql);
          $sql ="UPDATE sreetwear SET acheteur = '".$_SESSION['client']."' WHERE `collectibles`.`nom`= '".$_SESSION['item']."'";
          $result =mysqli_query($db_handle, $sql);   
        }
      }
    ...
    if ($db_found) {
      ...
      $sql = "SELECT prixActuel AND prix FROM collectibles ";
      $result = mysqli_query($db_handle, $sql);

      $sql ="UPDATE collectibles SET prixActuel = prix WHERE `collectibles`.`nom` = '".$_SESSION['item']."'";
      $result =mysqli_query($db_handle, $sql); 
      $sql ="UPDATE collectibles SET acheteur = '".$_SESSION['client']."' WHERE `collectibles`.`nom`= '".$_SESSION['item']."'";
      $result =mysqli_query($db_handle, $sql);    
    }
