  ...
  <div class="container" text-align="center">
    ...
    <div class="col-lg-9">
      ...
      <div class="card mt-4">
        ...
        <div class="card-body">
          ...
          <h4>Type de vente :
           ...
           $requete='SELECT * FROM `streetwear` WHERE nom= "'.$_SESSION['item'].'" ';
      ...
      <h4>DATE DE FIN  :
        ...
        $requete='SELECT `tempsRestant` FROM `streetwear` WHERE nom= "'.$_SESSION['item'].'" ';
        ...
        while ($ligne=$resultat->fetch_assoc()) {
         ...
         if ($dateActuel<=$ligne['tempsRestant'])
         {
          ...
          $sql ="UPDATE streetwear SET etat = '0' WHERE `streetwear`.`nom`= '".$_SESSION['item']."'";
          $result =mysqli_query($db_handle, $sql); 
...

        {
          ...
          $sql ="UPDATE streetwear SET etat = '1' WHERE `streetwear`.`nom` = '".$_SESSION['item']."'' ";
          $result =mysqli_query($db_handle, $sql);
        }
...
$db_handle = mysqli_connect('localhost', 'root', '');
$db_found = mysqli_select_db($db_handle, $database);
$requete= "SELECT prixActuel AND prix FROM streetwear ";
$resultat = mysqli_query($db_handle, $requete);
$requete2= "SELECT * FROM streetwear ";
...
while ($ligne=$resultat2->fetch_assoc()) {
  ...
  if ($ligne['etat']=='0'){
    ...
    if($b1) { // faire une offre
      if ($db_found) {
        if ($prixActuel<$ligne['prixActuel'])
        {
          ...
          $sql = "SELECT prixActuel FROM streetwear ";
          $result = mysqli_query($db_handle, $sql);

          $sql ="UPDATE streetwear SET prixActuel = '$prixActuel' WHERE `streetwear`.`nom`= '".$_SESSION['item']."'";
          $result =mysqli_query($db_handle, $sql);
          $sql ="UPDATE sreetwear SET acheteur = '".$_SESSION['client']."' WHERE `streetwear`.`nom`= '".$_SESSION['item']."'";
          $result =mysqli_query($db_handle, $sql);   
        }
      }
    ...
    if ($db_found) {
      ...
      $sql = "SELECT prixActuel AND prix FROM streetwear ";
      $result = mysqli_query($db_handle, $sql);

      $sql ="UPDATE streetwear SET prixActuel = prix WHERE `streetwear`.`nom` = '".$_SESSION['item']."'";
      $result =mysqli_query($db_handle, $sql); 
      $sql ="UPDATE streetwear SET acheteur = '".$_SESSION['client']."' WHERE `streetwear`.`nom`= '".$_SESSION['item']."'";
      $result =mysqli_query($db_handle, $sql);    
    }
